...
var smallbarwidth = 850 - barmargin.left - barmargin.right,
  smallbarheight = 250 - barmargin.top - barmargin.bottom;

// set the ranges
var smallbarx = d3.scaleBand().range([0, smallbarwidth]).padding(0.1);
var smallbary = d3.scaleLinear().range([smallbarheight, 0]);

var smallbar = d3
  .select("#small_bar")
  .append("svg")
  .attr("width", smallbarwidth + barmargin.left + barmargin.right)
  .attr("height", smallbarheight + barmargin.top + barmargin.bottom)
  .append("g")
  .attr("transform", "translate(" + barmargin.left + "," + barmargin.top + ")");
...
d3.csv("./data/change.csv").then(function (data) {
  ...
  smallbary.domain([0, 20]);

  // x Axis
  smallbar
    .append("g")
    .attr("transform", "translate(0," + smallbarheight + ")")
    .call(d3.axisBottom(smallbarx));

  smallbar.append("g").call(d3.axisLeft(smallbary).tickFormat((d) => d + "%"));
  ...
  smallbar
    ...
    .attr(
      ...
      d3
        .line()
        .x(function (d) {
          return smallbarx(d.year);
        })
        .y(function (d) {
          return smallbary(d.num);
        })
    );
